...
class CheckJobCondor:
  def __init__(self,cycles_before_exit_on_completed,cycles_before_exit_on_held,transfer_output,remove_job):
    self.arr = []
    self.job_ids =[]
    self.number_of_jobs=0
    self.JobStatus=['Unexpanded','Idle','Running','Removed','Completed','Held','Submission_err','Unknown']
    self.job_id=""
    self.job_status=[]
  ...
  def execute_condor_q(self):
    ...
    try:
        ...
        result = subprocess.check_output(["condor_q", "-af", "ClusterId"], universal_newlines=True)
  ...
  def check_single_job_status(self):
      ...
      try:
        ...
        print(command)
        # Run the "condor_q" command to list the Job IDs (Cluster IDs)                                                                           
        result = subprocess.check_output(["condor_q",command, "-af", "JobStatus"], universal_newlines=True)
    ...
    if self.job_status!=['']:
      ...
      for i in range(0,len(self.job_status)):
        ...
        if int(self.job_status[i]) <= 6:
          ...
          self.stats[int(self.job_status[i])]=self.stats[int(self.job_status[i])]+1
  ...
  def check_exit_condition(self):
    ...
    if self.counter_to_exit_completed==self.cycles_before_exit_on_completed or self.counter_to_exit_held==self.cycles_before_exit_on_held:
...
def main():
  ...
  p1 = CheckJobCondor(cycles_before_exit_on_completed=3,cycles_before_exit_on_held=3,transfer_output=sys.argv[1],remove_job=sys.argv[2]) 
